...
#define PIN_NUM_MISO TFT_SPI_MISO 
#define PIN_NUM_MOSI TFT_SPI_MOSI
#define PIN_NUM_CLK  TFT_SPI_SCLK
#define PIN_NUM_CS   TFT_SPI_CS

#define PIN_NUM_DC   TFT_DC
#define PIN_NUM_RST  TFT_RESET
#define PIN_NUM_BCKL TFT_BACKLIGHT
...
void st7789v_fill(int32_t x1, int32_t y1, int32_t x2, int32_t y2, lv_color_t color)
{
	...
	while(size > LV_HOR_RES) {
		lcd_data(buf, LV_HOR_RES * 2);
		size -= LV_HOR_RES;
	}

	lcd_data(buf, size * 2);	/*Send the remaining data*/
}
...
void st7789v_flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t * color_map)
{
	...
	lcd_data((void*)color_map, size * 2);	/*Send the remaining data*/
